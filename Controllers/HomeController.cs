...
namespace SchulbibliothekAP14.Controllers
{
    public class HomeController : Controller
    {
        ...
        public IActionResult Index(HomeViewmodel viewmodel)
        {
            ...
            if (!string.IsNullOrEmpty(viewmodel.Suche)) query = query.Where(t => t.Bemerkung.Contains(viewmodel.Suche));


            if (viewmodel.TransaktionTypId.HasValue) query = query.Where(t => t.TransaktionTypId == viewmodel.TransaktionTypId.Value);


            if (viewmodel.PersonId.HasValue) query = query.Where(t => t.PersonId == viewmodel.PersonId.Value);


            if (viewmodel.Anzahl.HasValue) query = query.Take(viewmodel.Anzahl.Value);
            ...
            }).OrderByDescending(li => li.Datum).ToList();
